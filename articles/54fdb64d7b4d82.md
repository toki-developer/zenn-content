...
SQLiteを使う準備をします。
SQLiteProviderの設定とテーブルの作成と初期値の設定をします。
今回はサンプルなので全て_layout.tsxに書いて進めます。
...
SQLiteを使う準備が完了したので、Homeページからデータを取得してみます。
...
app.jsonが更新されているので`npx expo prebuild -p ios --clean`を実行してから、`npm run ios`を実行します。

:::message
ライブラリを追加した時は、起動中の`npm run ios`を止めてから再度実行しています。
:::

アプリを開いて、`npm run ios`を実行したターミナル上に表示されたら成功です。
![](https://storage.googleapis.com/zenn-user-upload/2c4346b8a5e2-20251201.png)

これで、アプリ本体からSQLiteを使ってApp Groupのデータ取得まで出来ました。

### ウィジェットからApp Groupのファイルにアクセスする

ウィジェットからApp Groupを使うための設定をしていきます。
設定の詳細について[expo-apple-targets](https://github.com/EvanBacon/expo-apple-targets)にいろいろと記載があります。
...
```
※ app.jsonの値を参照して設定しています。

App Groupを参照してデータを取得するようにします。
...
:::message
自分の環境では一度XcodeとiOS Simulatorを終了しないとデータの取得が出来ませんでした。
うまくいかない場合は試してみてください。
それでも上手くいかない場合は、Xcodeからもビルドを実行してみてください。
:::

React側のmigrationファイルでinsertしたデータをウィジェットに表示することが出来ました。
![](https://storage.googleapis.com/zenn-user-upload/a3e239c71a2b-20251201.png)

これでアプリとウィジェットでデータの共有をすることが出来ました。
