---
title: "Expoでウィジェットを作る (アプリ本体とのデータ共有や即時反映まで)"
emoji: "✨"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["reactnative", "expo", "swift", "ios"]
published: true
published_at: 2025-12-11 06:00
---

## 概要

普段は業務でReactを書いています。
今回、React Nativeの勉強のため習慣アプリを作ってみました。その中でウィジェットの開発をしたので、作り方のメモをまとめていこうと思います。
アプリ本体とのデータ共有や、アプリでの変更をウィジェットにすぐ反映させる方法などについても触れています。
おまけでウィジェットでの環境変数の使い方なども書いています。

https://apps.apple.com/us/app/%E3%82%A6%E3%82%A3%E3%82%B8%E3%82%A7%E3%83%83%E3%83%88%E3%81%A7%E7%BF%92%E6%85%A3/id6754188081

この記事で利用するサンプルコードは以下です（実際にリリースした習慣アプリのコードではありません）。
https://github.com/toki-developer/zenn-sample-expo-widget

## 前提

* iOS固有のネイティブコードは、基本的に`/ios`ディレクトリで管理されます。
* ネイティブコードをすべて自身で管理するのは大変なので、Expo CNGでは、`/ios`ディレクトリを直接編集するのではなく、設定ファイルから自動生成するアプローチを取ります。
* `expo prebuild`コマンドを実行すると、アプリ構成ファイル（`app.json`や`app.config.js`）の情報を基に`/ios`ディレクトリが自動で更新・生成されます。
* `/ios`ディレクトリは`expo prebuild`を実行するたびに上書きされるので、独自のネイティブコードを追加したい場合は、別の場所にコードを配置して`prebuild`時にリンクさせる仕組みが必要です。

## プロジェクト作成と動作確認

:::message
XcodeやiOS Simulatorのセットアップは完了している前提で進めます。
:::

プロジェクトを作成します。
```bash
npx create-expo-app@latest zenn-widget-sample
```

作成したプロジェクトに移動します。
```bash
cd zenn-widget-sample
```

アプリを起動します。
```bash
npx expo prebuild -p ios
npm run ios
```

`npm run ios`を実行したターミナルには、以下のように表示されているはずです。`npm run ios`を実行してもiOS Simulatorが自動で起動しない場合は、ターミナルで`i`キーを押してください。

> › Press i │ open iOS simulator

アプリが起動できたら、ウィジェットの導入に進みます。

:::details アプリ起動のキャプチャ
![](https://storage.googleapis.com/zenn-user-upload/dbba31b5bccb-20251130.png)
:::

## ウィジェットの導入

### 利用するライブラリについて
現状、Expo公式ではウィジェット開発を標準サポートしていないため、Expo開発者のEvan Bacon氏によって開発された[expo-apple-targets](https://github.com/EvanBacon/expo-apple-targets)というライブラリ（Expo Config Plugin）を利用しました。

このライブラリを使うと、プロジェクトルートに`/targets/widget/`ディレクトリが作成されます。開発者はこのディレクトリ内のファイルを編集するだけでウィジェットを実装でき、`/ios`ディレクトリとのリンクは自動的に設定されます。

※ このライブラリは実験的な機能であり、将来的に破壊的変更が入る可能性もあります。しかし、これはウィジェット開発のためのSwiftライブラリではなく、Expoでウィジェットを利用可能にするためのものです。そのため、もし将来的に別のソリューションが登場した場合でも、ウィジェットのコード（Swift）自体は比較的容易に移行できると考えています。

### ライブラリを利用して導入する

[公式の手順](https://github.com/EvanBacon/expo-apple-targets)に従ってウィジェットの導入を進めていきます。


:::details 公式の手順
![](https://storage.googleapis.com/zenn-user-upload/05d601bdf678-20251130.png)

翻訳 (DeepL)
1. Expoプロジェクトでnpx create-targetを実行し、Appleターゲットを生成します。
2. 生成するターゲットを選択します。ウィジェットから始めることをお勧めします（例: npx create-target widget）。これにより、必要なウィジェットファイルがルートディレクトリの/targetsに生成され、@bacons/apple-targetsがインストールされ、Expo Configプラグインがプロジェクトに追加されます。
3. Apple Team IDが既にお分かりの場合は、app.json内のios.appleTeamIdプロパティで設定してください。Xcodeの「署名と機能」タブで確認でき、必要に応じて後から設定可能です。
4. npx expo prebuild -p ios --clean を実行してXcodeプロジェクトを生成します。
5. Xcode（xed ios）でプロジェクトを開き、expo:targets/<target>フォルダ内でウィジェットを開発できます。
6. ビルド準備が整ったら、Xcodeでターゲットを選択しビルドしてください。
:::

コマンドを実行してAppleターゲットを生成します。
```bash
npx create-target
```
ターゲットを選択するよう表示されるので「Widget」を選択します。
ネイティブに関わる`app.json`を更新したので、`prebuild`を実行してからアプリを起動します。
```bash
npx expo prebuild -p ios --clean
npm run ios
```

iOS Simulatorでホーム画面のアプリのアイコンを長押しすると、ウィジェットを追加するメニューが表示されるようになります。
試しにウィジェットを追加してみると、サンプルウィジェットが表示されることが確認できます。
|アイコンを長押し|ウィジェットを表示|
|-|-|
|![](https://storage.googleapis.com/zenn-user-upload/993ad6b12cad-20251130.png)|![](https://storage.googleapis.com/zenn-user-upload/cb57f9595086-20251130.png)|

### 修正して動作確認
Xcodeを開きます。
```bash
xed ios
```
![](https://storage.googleapis.com/zenn-user-upload/c7d01679614d-20251130.png)

`expo:targets/widget`ディレクトリの中が修正する箇所になります。

試しに `expo:targets/widget/widgets.swift`を修正してみます。
```diff swift:expo:targets/widget/widgets.swift
    var body: some View {
        VStack {
            Text("Time:")
            Text(entry.date, style: .time)

+            Text("Hello Widget")
-            Text("Favorite Emoji:")
-            Text(entry.configuration.favoriteEmoji)
        }
    }
```

Swiftファイルを修正したので、`npm run ios`でアプリを再ビルドします。
今回は`app.json`を修正していないので、`prebuild`は不要です。
```bash
npm run ios
```

:::message
既に`npm run ios`を実行中の場合、プロセスを停止せずに別のターミナルで再度`npm run ios`を実行すると、開発サーバーの起動が不要なため、ビルド時間を短縮できます。
:::

ウィジェットが変更されていることが確認できました。

![](https://storage.googleapis.com/zenn-user-upload/205ae14a762f-20251130.png)

これでウィジェットの開発を始める準備が完了しました。

## ウィジェットのコードを確認
### 自動生成されたコード
`npx create-target`で自動生成されたコードを見ていきましょう。
以下の5つのSwiftファイルが生成されています。
* `index.swift`
  * ルートファイルです。
  * `widget`、`WidgetControl`、`WidgetLiveActivity`を呼び出しています。
  * 新しいウィジェットを作成した場合は、ここに追加します。
* `WidgetControl.swift`
  * コントロールセンターなどに追加できる「Control」を提供するファイルです。
  * 今回の記事では対象外とします（未検証）。
* `WidgetLiveActivity.swift`
  * ロック画面などにリアルタイムの情報を表示できる「Live Activity」を定義するファイルです。
  * 今回の記事では対象外とします（未検証）。
* `widget.swift`
  * 先ほど確認したような通常のウィジェット機能用のファイルです。
  * 今回の記事では、このウィジェットを対象とします。
* `AppIntent.swift`
  * ウィジェットの設定用の`AppIntent`を定義するファイルです。
  * ウィジェット単位で値を設定する機能をユーザーに提供できます。
  * ユーザーはウィジェット設定画面から値を設定し、設定した値を元にウィジェットの表示をすることが出来ます。

今回はホーム画面に表示する通常のウィジェットのみを扱うため、`WidgetControl.swift`と`WidgetLiveActivity.swift`の修正は不要です。
`index.swift`からも関連するコードを削除します。

```diff swift:expo:targets/widget/index.swift
@main
struct exportWidgets: WidgetBundle {
    var body: some Widget {
        // Export widgets here
        widget()
-        widgetControl()
-        WidgetLiveActivity()
    }
}
```

:::details 余談: コントロールセンターを確認
`widgetControl`の呼び出しを消す前はコントロールセンターに以下のようなアイコンが表示されますが、消すと表示されなくなります。
![](https://storage.googleapis.com/zenn-user-upload/051323302c55-20251130.png)
:::

### 通常のウィジェット関連のコードの詳細
主に`AppIntent.swift`と`widget.swift`が修正対象のコードです。

#### AppIntent.swift
* `WidgetConfigurationIntent`
  * ウィジェットの編集画面から設定可能なパラメータなどを定義します。
  * `widget.swift`から利用しています。

```diff swift:expo:targets/widget/AppIntent.swift
    @Parameter(title: "Favorite Emoji", default: "😃")
    var favoriteEmoji: String
```

|ウィジェットを長押し|「ウィジェットを編集」をタップ|
|-|-|
|![](https://storage.googleapis.com/zenn-user-upload/20227c84e4b4-20251130.png)|![](https://storage.googleapis.com/zenn-user-upload/154cc1277581-20251130.png)|

#### widget.swift
* `widget`
  * `index.swift`から呼び出される最上位の構造体を定義します。
* `View` (`widgetEntryView`)
  * ウィジェットのUI部分です。
* `Entry` (`SimpleEntry`)
  * 表示するデータや設定値を持つ構造体です。
* `AppIntentTimelineProvider`
  * `placeholder`
    * データがまだ取得できない場合などに表示される`Entry`を定義します。
  * `snapshot`
    * ウィジェット追加時に表示されるサンプルデータの`Entry`を定義します。
  * `timeline`
    * データを更新するタイミング（例: 5分ごと、1時間ごとなど）と、実際にウィジェットに表示するデータを定義します。

詳細については、以下の記事が参考になりました。
https://qiita.com/yuuka_____gf/items/604c29a71584206f23d3

## ウィジェットを実装してみる

データ取得用のクラスを作成し、サンプルデータを表示するまでの実装を行います。

### データ取得用のクラスを作成

`expo:targets/widget/DataManager.swift`ファイルを作成します。

```swift:expo:targets/widget/DataManager.swift
struct SampleModel: Identifiable {
  var id: String
  var name: String
}

class DataManager {
  static let shared = DataManager()
  
  func loadSampleData() -> [SampleModel] {
    let data = [
      SampleModel(id: "1", name: "hoge"),
      SampleModel(id: "2", name: "fuga")
    ]
    
    return data
  }
}
```

### データを取得して表示する

`DataManager`が返す`SampleModel`の配列を`SimpleEntry`に追加します。
`View`は、この`SimpleEntry`のデータを使ってUIを構築します。

ウィジェットの更新タイミングは、毎日0時に設定しました。

```diff swift:expo:targets/widget/widget.swift
func placeholder(in context: Context) -> SimpleEntry {
-   SimpleEntry(date: Date(), configuration: ConfigurationAppIntent())
+   SimpleEntry(date: Date(), sampleList: [], configuration: ConfigurationAppIntent())
}

func snapshot(for configuration: ConfigurationAppIntent, in context: Context) async -> SimpleEntry {
-   SimpleEntry(date: Date(), configuration: ConfigurationAppIntent())
+   SimpleEntry(date: Date(), sampleList: [], configuration: ConfigurationAppIntent())
}
func timeline(for configuration: ConfigurationAppIntent, in context: Context) async -> Timeline<SimpleEntry> {
-    var entries: [SimpleEntry] = []
-
-    // Generate a timeline consisting of five entries an hour apart, starting from the current date.
-    let currentDate = Date()
-    for hourOffset in 0 ..< 5 {
-        let entryDate = Calendar.current.date(byAdding: .hour, value: hourOffset, to: currentDate)!
-        let entry = SimpleEntry(date: entryDate, configuration: configuration)
-        entries.append(entry)
-    }
-
-    return Timeline(entries: entries, policy: .atEnd)
+    let data = DataManager.shared.loadSampleData()
+    let entry = SimpleEntry(date: Date(), sampleList: data, configuration: configuration)
+
+    // 毎日0時に更新
+    let calendar = Calendar.current
+    let now = Date()
+    let nextMidnight = calendar.nextDate(
+        after: now,
+        matching: DateComponents(hour: 0, minute: 0),
+        matchingPolicy: .nextTime
+    )!
+
+    return Timeline(entries: [entry], policy: .after(nextMidnight))
}

struct SimpleEntry: TimelineEntry {
    let date: Date
+   let sampleList: [SampleModel]
    let configuration: ConfigurationAppIntent
}

struct widgetEntryView : View {
    var entry: Provider.Entry

    var body: some View {
        VStack {
-           Text("Time:")
-           Text(entry.date, style: .time)
-
-           Text("Hello Widget")
+           ForEach(entry.sampleList, id: \.id) { sample in
+               Text(sample.name)
+           }
        }
    }
}
```

修正後、`npm run ios`コマンドでアプリをビルドします。
取得したデータを表示することができました。

![](https://storage.googleapis.com/zenn-user-upload/dac1350d0fa9-20251130.png)

ここまでの実装が完了すれば、あとは`DataManager.swift`で実際に表示したいデータを取得するように修正し、`widget.swift`の`View`を好みのデザインに編集することで、オリジナルのウィジェットを構築できます。

## データをアプリ本体と共有する

### 概要
アプリ本体とウィジェット間でデータをやり取りしたい場合には、App Groupがよく使われます。
App Groupは、隔離されたサンドボックス環境を超えて安全にデータを共有できるようにする仕組みです。
私が作成した習慣アプリは、データをローカルのSQLiteに保存しています。この経験を元に、今回のサンプルでもアプリ本体とウィジェットで共有のSQLiteファイルにアクセスする方法を解説します。
他には`UserDefaults`などがよく使われるようです。

### アプリ本体からApp Groupのファイルにアクセスする

まず、React Native側でSQLiteを使う準備をします。
Expo公式のドキュメントに従って進めていきます。

https://docs.expo.dev/versions/latest/sdk/sqlite/

ライブラリをインストールします。
```bash
npx expo install expo-sqlite
```

`SQLiteProvider`の設定、テーブル作成、初期データの投入を行います。
今回はサンプルとして、これらの設定をすべて`app/_layout.tsx`に記述します。

```diff tsx:app/_layout.tsx
+ import { SQLiteProvider, type SQLiteDatabase } from "expo-sqlite";
+ import { Paths } from "expo-file-system";

+ const APP_GROUP_ID = "group.com.toki-developer.zenn-widget-sample.widget";

export default function RootLayout() {
  const colorScheme = useColorScheme();

+  // App Groupのパスを取得
+  const dbDirectory = useMemo(() => {
+    return Paths.appleSharedContainers[APP_GROUP_ID]?.uri;
+  }, []);

  return (
    <ThemeProvider value={colorScheme === "dark" ? DarkTheme : DefaultTheme}>
+      <SQLiteProvider databaseName="database.db" directory={dbDirectory} onInit={migrateDbIfNeeded}>
        <Stack>
          <Stack.Screen name="(tabs)" options={{ headerShown: false }} />
          <Stack.Screen
            name="modal"
            options={{ presentation: "modal", title: "Modal" }}
          />
        </Stack>
        <StatusBar style="auto" />
+      </SQLiteProvider>
    </ThemeProvider>
  );
}

+ async function migrateDbIfNeeded(db: SQLiteDatabase) {
+   const DATABASE_VERSION = 1;
+   let currentDbVersion;
+   const res = await db.getFirstAsync<{
+       user_version: number;
+   }>("PRAGMA user_version");
+   currentDbVersion = res ? res.user_version : 0;
+   if (currentDbVersion >= DATABASE_VERSION) {
+       return;
+   }
+   if (currentDbVersion === 0) {
+        // テーブル作成
+       await db.execAsync(`
+           CREATE TABLE sample (
+           id INTEGER PRIMARY KEY,
+           name TEXT NOT NULL
+       );
+       `);
+
+       // 初期データ挿入
+       await db.execAsync(`
+           INSERT INTO sample (id, name) VALUES
+           (1, 'hoge'),
+           (2, 'Hello Widget');
+       `);
+
+       currentDbVersion = 1;
+   }
+   await db.execAsync(`PRAGMA user_version = ${DATABASE_VERSION}`);
+}
```

SQLiteの準備ができたので、Home画面からデータを取得してみましょう。

```diff tsx:app/(tabs)/index.tsx
// ...
+ import { useSQLiteContext } from "expo-sqlite";
+ import { useEffect } from "react";

export default function HomeScreen() {
+  const db = useSQLiteContext();
+
+  useEffect(() => {
+    async function fetchData() {
+      const results = await db.getAllAsync<{ id: number; name: string }>(
+        "SELECT * FROM sample"
+      );
+      console.log("Sample table data:", results);
+    }
+    fetchData();
+  }, [db]);

  return (
// ...
```

`app.json`を更新したので、`npx expo prebuild -p ios --clean`を実行してから`npm run ios`でアプリを起動します。

:::message
ライブラリを追加した際は、起動中の`npm run ios`を一度停止してから再度実行しています。
:::

アプリを開き、`npm run ios`を実行したターミナルにログが表示されたら成功です。
![](https://storage.googleapis.com/zenn-user-upload/2c4346b8a5e2-20251201.png)

これで、アプリ本体からApp Group内の共有DBにアクセスし、データを取得できるようになりました。

### ウィジェットからApp Groupのファイルにアクセスする

次に、ウィジェット側からApp Groupの共有DBにアクセスするための設定を行います。
設定の詳細は[expo-apple-targetsのリポジトリ](https://github.com/toki-developer/zenn-sample-expo-widget/blob/main/modules/widget-update/src/WidgetUpdateModule.ts)に記載されています。

```diff json:app.json
    "ios": {
      "supportsTablet": true,
      "bundleIdentifier": "com.toki-developer.zenn-widget-sample",
+      "entitlements": {
+        "com.apple.security.application-groups": ["group.com.toki-developer.zenn-widget-sample.widget"]
+      }
    },
```
```diff js:targets/widget/expo-target.config.js
module.exports = config => ({
  type: "widget",
  icon: 'https://github.com/expo.png',
+  entitlements: {
+    "com.apple.security.application-groups":
+      config.ios.entitlements["com.apple.security.application-groups"],
+  },
});
```
※ `expo-target.config.js`では`app.json`の値を参照して設定しています。

`DataManager.swift`を修正し、App Group内のDBからデータを取得するようにします。
```swift:targets/widget/DataManager.swift
import Foundation
import SQLite3

struct SampleModel: Identifiable {
  var id: String
  var name: String
}

class DataManager {
  static let shared = DataManager()
  
  func loadSampleData() -> [SampleModel] {
    guard let db = DatabaseManager.shared.openDatabase() else { return [] }
    defer { DatabaseManager.shared.closeDatabase() }
    
    return getSampleList(db: db)
  }
  
  private func getSampleList(db: OpaquePointer) -> [SampleModel] {
    var results: [SampleModel] = []
    let query = "SELECT id, name FROM sample;"
    var statement: OpaquePointer?

    if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
        while sqlite3_step(statement) == SQLITE_ROW {
            let id = String(cString: sqlite3_column_text(statement, 0))
            let name = String(cString: sqlite3_column_text(statement, 1))
            results.append(SampleModel(id: id, name: name))
        }
    }
    sqlite3_finalize(statement)
    
    return results
  }
}

class DatabaseManager {
    static let shared = DatabaseManager()
    
    let dbPath: String
    private var db: OpaquePointer?
    
    private init() {
        let appGroupId = "group.com.toki-developer.zenn-widget-sample.widget"

        guard let container = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroupId) else {
            fatalError("App Group設定エラー")
        }
        
        self.dbPath = container.appendingPathComponent("database.db").path
    }
    
    func openDatabase() -> OpaquePointer? {
        if !FileManager.default.fileExists(atPath: dbPath) {
            print("データベースファイルが存在しません: \(dbPath)")
            return nil
        }
        
        if sqlite3_open(dbPath, &db) == SQLITE_OK {
            return db
        } else {
            print("SQLite接続エラー: \(String(cString: sqlite3_errmsg(db)))")
            return nil
        }
    }
    
    func closeDatabase() {
        if db != nil {
            sqlite3_close(db)
            db = nil
        }
    }
}
```

`npx expo prebuild -p ios --clean`と`npm run ios`を実行して確認します。

:::message
私の環境では、一度XcodeとiOS Simulatorを完全に終了させないと、App Groupのデータがうまく反映されませんでした。
うまくいかない場合は、これらを試してみてください。
それでもうまくいかない場合は、Xcodeから直接ビルドすることも試してみてください。
:::

アプリ本体側で`INSERT`したデータが、ウィジェットに正しく表示されることを確認できました。
![](https://storage.googleapis.com/zenn-user-upload/a3e239c71a2b-20251201.png)

これで、アプリとウィジェット間でデータを共有する仕組みが完成しました。

## アプリからウィジェットを更新する

続いて、アプリ側でデータが更新されたタイミングで、ウィジェットの表示も即時更新する仕組みを実装します。

ウィジェットの更新をするには、[WidgetCenter](https://developer.apple.com/documentation/widgetkit/widgetcenter)を利用します。

全てのウィジェットを更新する場合は`reloadAllTimelines()`を実行します。
特定のウィジェットだけを更新したい場合は`reloadTimelines(ofKind:)`を実行します。

```swift
WidgetCenter.shared.reloadAllTimelines()
// or
WidgetCenter.shared.reloadTimelines(ofKind: "widget") // "widget"はウィジェットのKind
```

この`WidgetCenter`の機能をReact Nativeから呼び出すために、Expo Modules APIを利用します。

プロジェクトのルートディレクトリで以下のコマンドを実行します。
```bash
npx create-expo-module@latest --local widget-update
```

`/modules/widget-update/`ディレクトリが作られます。

![](https://storage.googleapis.com/zenn-user-upload/b5d207defbdd-20251201.png)

このディレクトリにSwiftのコードを書くことで、React Nativeからネイティブ機能を呼び出せるようになります。
Expo Modules APIについて、以前こちらの記事で紹介しましたので気になったら参考にしてください。
https://zenn.dev/toki_dev/articles/7362fab854aec3

まず、ウィジェットのタイムラインを更新するネイティブ関数を定義します。
`modules/widget-update/ios/WidgetUpdateModule.swift`を以下のように編集します。

```diff swift:modules/widget-update/ios/WidgetUpdateModule.swift
import ExpoModulesCore
+import WidgetKit

  public func definition() -> ModuleDefinition {
    Name("WidgetUpdate")

+    AsyncFunction("reloadTimelines") {
+      DispatchQueue.main.async {
+          WidgetCenter.shared.reloadAllTimelines()
+      }
+    }
  }
```

次に対応するTypeScriptのコードを修正します。`modules/widget-update/src/WidgetUpdateModule.ts`を編集して、ネイティブ関数を呼び出せるようにします。

```diff ts:modules/widget-update/src/WidgetUpdateModule.ts
declare class WidgetUpdateModule extends NativeModule<WidgetUpdateModuleEvents> {
  PI: number;
  hello(): string;
  setValueAsync(value: string): Promise<void>;
+ reloadTimelines(): void;
}
```
`PI`、`hello`など `reloadTimelines`以外のものは消してもかまいません。

あとは、このモジュールをReact Native側から呼び出すだけです。
サンプルとして、DBのデータを更新し、同時にウィジェットの更新をトリガーするボタンを配置します。
```diff tsx:app/(tabs)/index.tsx
import WidgetUpdateModule from "@/modules/widget-update/src/WidgetUpdateModule";

    <Button
        title="更新"
        onPress={async () => {
            await db.execAsync(
            `UPDATE sample SET name = 'Hello Zenn' WHERE id = 1`
            );
            WidgetUpdateModule.reloadTimelines();
        }}
    />
```

`npx expo prebuild -p ios --clean`と`npm run ios`を実行して確認します。

ボタンをタップしてからホーム画面に戻ると、ウィジェットの表示が更新されていることを確認できました。
`reloadTimelines()`の呼び出しをコメントアウトした場合に、ウィジェットが更新されないことも確認しておくと、処理の流れがより明確に理解できるでしょう。

![](https://storage.googleapis.com/zenn-user-upload/3ec7082212f4-20251201.png)

これで、アプリからウィジェットを更新することができるようになりました。
今回の記事は以上です。これらの手順で、Expoアプリにウィジェットを導入し、アプリと連携させることができるようになったかと思います。

読んでいただきありがとうございました！

## おまけ

### ウィジェットからデータを更新する
今回の記事では詳細を省きますが、ウィジェット側からデータを更新することも可能です。
`AppIntent`を使って、ウィジェット上のボタンタップなどをトリガーに処理を実行できます。

```swift
import AppIntents

struct ActionIntent: AppIntent {
    static var title: LocalizedStringResource = "Action"

    @Parameter(title: "ID")
    var id: String

    init(id: String) {
        self.id = id
    }
    
    init() {
        self.id = ""
    }

    @MainActor
    func perform() async throws -> some IntentResult {
        // ここでデータ更新などの処理を実行する
        // ...
        
        // 処理後にウィジェットのタイムラインを更新
        WidgetCenter.shared.reloadAllTimelines()
        
        return .result()
    }
}
```

```swift
Button(intent: ActionIntent(id: "some_id")) {
    Text("Update Data")
}
```

ウィジェット側でデータを更新した際に、アプリ本体の表示も同期させたい場合は、`AppState`を利用してアプリがフォアグラウンドになったことを検知し、データを再取得するなどの実装が考えられます。

```ts:.ts
const subscription = AppState.addEventListener(
    "change",
    handleAppStateChange // データを再取得する処理を書く
);
```

### ウィジェットから環境変数を使う

同じデバイスに開発版と本番版を別アプリとしてインストールしたい場合は、以下のドキュメントが参考になります。
https://docs.expo.dev/build-reference/variants/

環境を分けたら環境変数を使いたくなることがあると思います。
以下のコードで、`Info.plist`の値を参照することが出来ます。
```swift:.swift
Bundle.main.object(forInfoDictionaryKey: "appGroupId") as? String
```

```plist:targets/widget/Info.plist
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
  <dict>
    <key>NSExtension</key>
    <dict>
      <key>NSExtensionPointIdentifier</key>
      <string>com.apple.widgetkit-extension</string>
    </dict>
    <key>appGroupId</key>
    <string>group.com.toki-developer.zenn-widget-sample.widget</string>
  </dict>
</plist>
```

[こちらのIssue](https://github.com/EvanBacon/expo-apple-targets/issues/106#issuecomment-2738197338)で紹介されているConfig Pluginを参考に、ビルド（`prebuild`）時に環境に応じた値を`Info.plist`に書き込むようにしています。

:::details 導入例

```js:plugins/withMultipleTargetInfoPlist/plugin.js
const { withDangerousMod } = require("@expo/config-plugins");
const fs = require("fs");
const path = require("path");
const plist = require("plist"); // Remember to install this module

const withMultipleTargetInfoPlist = (config, { targets }) => {
  return withDangerousMod(config, [
    "ios",
    (config) => {
      const projectRoot = config.modRequest.projectRoot;
      const iosDir = path.join(projectRoot, "targets");
      console.log("[👀 Customizing Info.plist] Targets directory:", iosDir);
      for (const { targetName, infoPlistUpdates } of targets) {
        const targetPath = path.join(iosDir, targetName, "Info.plist");
        console.log(
          "[👀 Customizing Info.plist] Found target:",
          targetName,
          "at",
          targetPath
        );
        if (fs.existsSync(targetPath)) {
          console.log(
            "[🤖 Customizing Info.plist] Matched target:",
            targetName,
            ", updating Info.plist"
          );
          const plistContent = fs.readFileSync(targetPath, "utf-8");
          const parsedPlist = plist.parse(plistContent);
          Object.assign(parsedPlist, infoPlistUpdates);
          const updatedPlistContent = plist.build(parsedPlist);
          fs.writeFileSync(targetPath, updatedPlistContent, "utf-8");
          console.log("[✅ Customizing Info.plist] Plist updated");
        } else {
          console.log(
            "[🟠 Customizing Info.plist] Target path does not exist, skipping"
          );
        }
      }

      return config;
    },
  ]);
};

module.exports = withMultipleTargetInfoPlist;
```

```js:app.config.js
export default {
  expo: {
    // ...
    plugins: [
        //...
        ,
      [
        "./plugins/withMultipleTargetInfoPlist/plugin.js",
        {
          targets: [
            {
              targetName: "widget",
              infoPlistUpdates: {
                appGroupId: appGroupId,
                scheme: scheme,
              },
            },
          ],
        },
      ],
    ]
  }
}
```

:::



## 参考

https://docs.expo.dev/workflow/continuous-native-generation
https://github.com/EvanBacon/expo-apple-targets
https://tech.fusic.co.jp/posts/expo-apple-widget/
https://qiita.com/yuuka_____gf/items/604c29a71584206f23d3
https://github.com/EvanBacon/expo-apple-targets/issues/106
https://docs.expo.dev/build-reference/variants/
https://zenn.dev/toki_dev/articles/7362fab854aec3
https://docs.expo.dev/versions/latest/sdk/sqlite/